...
import { createMySQLEngine } from "./engine.js";
...
describe("createMySQLEngine", () => {
  ...
  beforeEach(() => {
    engine = createMySQLEngine({ client: pool });
  });
  ...
  describe("execute", () => {
    it("should execute DDL statements", async () => {
      await engine.execute("CREATE TABLE test (id INT AUTO_INCREMENT PRIMARY KEY, name TEXT)");
    });
    ...
    it("should support parameterized queries", async () => {
      await engine.execute(
        "CREATE TABLE test (id INT AUTO_INCREMENT PRIMARY KEY, name TEXT, age INT)",
      );
  ...
  describe("error wrapping", () => {
    ...
    it("should include the original error message", async () => {
      try {
        ...
        expect((err as AlreadyExistsError).message).toContain("Duplicate entry");
      }
    });
  });
  ...
  describe("close", () => {
    it("should close the pool", async () => {
      ...
      const testEngine = createMySQLEngine({ client: testPool });
      await testEngine.close();
      await expect(testPool.execute("SELECT 1")).rejects.toThrow();
    });
  });
});
