import type { DescField, DescMessage, MessageInitShape, MessageShape } from "@bufbuild/protobuf";
import { clone, create, merge } from "@bufbuild/protobuf";
import { FieldMaskSchema, timestampNow } from "@bufbuild/protobuf/wkt";
...
/**
 * Extract a simple `Record<string, string>` column name mapping from
 * the richer {@link ColumnConfig} map. Returns `undefined` when no
 * name overrides are present.
 */
...
/**
 * Chain interceptors around a core function. The first interceptor in
 * the array is outermost (runs first on the way in, last on the way out).
 *
 * Generic over `R` so callers get the correct return type without casting.
 */
function applyInterceptors<Desc extends DescMessage, R>(
...
export function createRepository<Desc extends DescMessage>(
  ...
  const columnMap = resolveColumnMap(columnConfigs);
  ...
  const etagMask = opts.etag?.mask;
  const computeEtag =
    opts.etag?.fn ??
    ((s: Desc, m: MessageShape<Desc>) =>
      defaultEtagFn(s, m, etagMask ? { fieldMask: etagMask } : undefined));
  const identifierFields = schema.fields.filter((f) =>
    hasFieldBehavior(f, FieldBehavior.IDENTIFIER),
  );

  // Resolve timestamp columns from columnConfigs
  const createTimestampFields: DescField[] = [];
  const updateTimestampFields: DescField[] = [];
  if (columnConfigs) {
    for (const [fieldName, config] of Object.entries(columnConfigs)) {
      if (!config.timestamp) continue;
      const field = schema.fields.find((f) => f.name === fieldName);
      if (!field) continue;
      if (config.timestamp === "create" || config.timestamp === "update") {
        createTimestampFields.push(field);
      }
      if (config.timestamp === "update") {
        updateTimestampFields.push(field);
      }
    }
  }
  ...
  return {
    ...
    async get(query: QueryInput<Desc>, options?: GetOptions): Promise<MessageShape<Desc>> {
      return applyInterceptors(
        interceptors,
        { operation: "get", schema, tableName, query, options },
        async () => {
          ...
          let message = deserializeRow(schema, row, columnMap, columnConfigs);
    ...
    async create(
      ...
      return applyInterceptors(
        interceptors,
        { operation: "create", schema, tableName, resource, options },
        async () => {
          const eng: Engine = options?.transaction ?? engine;
          const readMask = options?.readMask ?? defaultReadMask;
          ...
          // Auto-populate create and update timestamps
          for (const f of createTimestampFields) {
            (msg as Record<string, unknown>)[f.localName] = timestampNow();
          }
          ...
          const row = serializeMessage(schema, msg, columnMap, columnConfigs);
          ...
          let result = deserializeRow(schema, inserted, columnMap, columnConfigs);
    ...
    async list(query?: QueryInput<Desc>, options?: ListOptions): Promise<ListResult<Desc>> {
      return applyInterceptors(
        interceptors,
        { operation: "list", schema, tableName, query, options },
        async () => {
          const eng: Engine = options?.transaction ?? engine;
          const readMask = options?.readMask ?? defaultReadMask;
          ...
          let results = pageRows.map((row) =>
            deserializeRow(schema, row, columnMap, columnConfigs),
          );
          if (!isWildcard(readMask)) {
    ...
    async update(
      ...
      return applyInterceptors(
        interceptors,
        { operation: "update", schema, tableName, query, resource, options },
        async () => {
          const eng: Engine = options?.transaction ?? engine;
          const readMask = options?.readMask ?? defaultReadMask;
          ...
          const existing = deserializeRow(schema, existingRow, columnMap, columnConfigs);
          ...
          // Auto-populate update timestamps
          for (const f of updateTimestampFields) {
            (merged as Record<string, unknown>)[f.localName] = timestampNow();
          }

          // Recompute etag on merged message
          if (etagFieldDesc) {
            (merged as Record<string, unknown>)[etagFieldDesc.localName] = computeEtag(
              schema,
              merged,
            );
          }
          ...
          const fullRow = serializeMessage(schema, merged, columnMap, columnConfigs);
          ...
          let result = deserializeRow(schema, updatedRow, columnMap, columnConfigs);
          if (!isWildcard(readMask)) {
            result = applyFieldMask(schema, result, readMask, { strict: false });
          }
          return result;
    ...
    async delete(query: QueryInput<Desc>, options?: DeleteOptions): Promise<void> {
      return applyInterceptors(
        interceptors,
        { operation: "delete", schema, tableName, query, options },
        async () => {
          const eng: Engine = options?.transaction ?? engine;
          const filter = buildFilter(schema, query, { columnMap, extraDecls: filterDecls });
    ...
    async count(query?: QueryInput<Desc>, options?: CountOptions): Promise<number> {
      return applyInterceptors(
        interceptors,
        { operation: "count", schema, tableName, query, options },
        async () => {
          const eng: Engine = options?.transaction ?? engine;
          const filter = query
            ? buildFilter(schema, query, { columnMap, extraDecls: filterDecls })
    ...
    async batchGet(
      ...
      return applyInterceptors(
        interceptors,
        { operation: "batchGet", schema, tableName, queries, options },
        async () => {
          const eng: Engine = options?.transaction ?? engine;
          const readMask = options?.readMask ?? defaultReadMask;
          ...
          const run = async (e: Engine) => {
            ...
            let results = rows.map((row) => deserializeRow(schema, row, columnMap, columnConfigs));
            ...
            if (identifierFields.length > 0) {
              ...
              for (const query of queries) {
                ...
                if (typeof query !== "string") {
                  ...
                  const msg = deserializeRow(schema, row, columnMap, columnConfigs);
                  key = identifierFields
                    .map((f) => String((msg as Record<string, unknown>)[f.localName]))
                    .join("\0");
                }
    ...
    async batchCreate(
      ...
      return applyInterceptors(
        interceptors,
        { operation: "batchCreate", schema, tableName, resources, options },
        async () => {
          const eng: Engine = options?.transaction ?? engine;
          const readMask = options?.readMask ?? defaultReadMask;
          ...
          const run = async (e: Engine) => {
            ...
            // Phase 3: Auto-populate timestamps and compute etags
            for (const msg of messages) {
              for (const f of createTimestampFields) {
                (msg as Record<string, unknown>)[f.localName] = timestampNow();
              }
              if (etagFieldDesc) {
                (msg as Record<string, unknown>)[etagFieldDesc.localName] = computeEtag(
                  schema,
                  msg,
                );
              }
            ...
            const rows = messages.map((msg) =>
              serializeMessage(schema, msg, columnMap, columnConfigs),
            );
            ...
            // Deserialize and apply readMask
            let results = insertedRows.map((row) =>
              deserializeRow(schema, row, columnMap, columnConfigs),
            );
            if (!isWildcard(readMask)) {
              results = results.map((msg) =>
    ...
    async batchUpdate(
      ...
      return applyInterceptors(
        interceptors,
        { operation: "batchUpdate", schema, tableName, updates, options },
        async () => {
          const eng: Engine = options?.transaction ?? engine;
          const readMask = options?.readMask ?? defaultReadMask;
          ...
          const run = async (e: Engine) => {
            ...
            for (const update of updates) {
              ...
              if (!existing) {
                ...
                existing = deserializeRow(schema, row, columnMap, columnConfigs);
              }

              validateImmutableFields(schema, updateMsg, { fieldMask: updateMask });

              const effectiveMask = isWildcard(updateMask) ? outputOnlyMask(schema) : updateMask;
              const maskedUpdate = applyFieldMask(schema, updateMsg, effectiveMask, {
                strict: false,
              });
              ...
              // Auto-populate update timestamps
              for (const f of updateTimestampFields) {
                (result as Record<string, unknown>)[f.localName] = timestampNow();
              }
            ...
            let results = updatedRows.map((row) =>
              deserializeRow(schema, row, columnMap, columnConfigs),
            );
            if (!isWildcard(readMask)) {
              results = results.map((msg) =>
    ...
    async batchDelete(queries: QueryInput<Desc>[], options?: BatchDeleteOptions): Promise<void> {
      return applyInterceptors(
        interceptors,
        { operation: "batchDelete", schema, tableName, queries, options },
        async () => {
          const eng: Engine = options?.transaction ?? engine;
