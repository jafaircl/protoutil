...
import { astToAlphaCheckedExpr, env, StringType, variable } from "@protoutil/cel";
...
describe("postgres — user-provided functions", () => {
  ...
  it("custom function not in stdlib is dispatched correctly", () => {
    expect(
      postgres(checked(`title.fuzzy("dragon")`, fuzzyDecls), {
        functions: {
  ...
  it("unknown function with no handler throws TranslationError", () => {
    expect(() => postgres(checked(`title.fuzzy("dragon")`, fuzzyDecls))).toThrow(
