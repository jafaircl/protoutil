...
describe('ConstantsTest', () => {
  ...
  it('parseInt_base10Max', () => {
    ...
    expect(constant.constantKind.case).toEqual('int64Value');
    expect(constant.constantKind.value).toEqual(BigInt(Number.MAX_SAFE_INTEGER));
  });

  it('parseInt_base10Min', () => {
    const constant = parseIntConstant(Number.MIN_SAFE_INTEGER.toString());
    expect(constant.constantKind.case).toEqual('int64Value');
    expect(constant.constantKind.value).toEqual(BigInt(Number.MIN_SAFE_INTEGER));
  });
  ...
  it('parseInt_base16Max', () => {
    const constant = parseIntConstant(`0x${Number.MAX_SAFE_INTEGER.toString(16)}`);
    expect(constant.constantKind.case).toEqual('int64Value');
    expect(constant.constantKind.value).toEqual(BigInt(Number.MAX_SAFE_INTEGER));
  });

  it('parseInt_base16Min', () => {
    const constant = parseIntConstant(`-0x${(-1 * Number.MIN_SAFE_INTEGER).toString(16)}`);
    expect(constant.constantKind.case).toEqual('int64Value');
    expect(constant.constantKind.value).toEqual(BigInt(Number.MIN_SAFE_INTEGER));
  });
  ...
  it('parseUint_base16Max', () => {
    let constant = parseUintConstant(`0x${Number.MAX_SAFE_INTEGER.toString(16)}u`);
    expect(constant.constantKind.case).toEqual('uint64Value');
    expect(constant.constantKind.value).toEqual(BigInt(Number.MAX_SAFE_INTEGER));
  ...
  it('parseBytes_throwsOnShortUnicodeEscapeSequence', () => {
    expect(() => parseBytesConstant('b"\\u0068\\u0065\\u006c\\u006c\\u006f"')).toThrow(
      ParseException
    );
  });
  ...
  it('parseBytes_throwsOnLongUnicodeEscapeSequence', () => {
    expect(() =>
      parseBytesConstant('b"\\U00000068\\U00000065\\U0000006c\\U0000006c\\U0000006f"')
  ...
  function testBytes(actual: string, expected: string) {
    ...
    expect(constant.constantKind.value).toEqual(new TextEncoder().encode(expected));
  }
