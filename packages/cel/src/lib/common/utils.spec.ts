import { isNil, safeParseFloat, safeParseInt, unquote } from './utils.js';
  ...
  for (const testCase of safeParseIntTestCases) {
    if (isNil(testCase.expected)) {
      it(`safeParseInt - should throw for ${testCase.input}`, () => {
        expect(() => safeParseInt(testCase.input, testCase.bits, testCase.signed)).toThrow();
      });
    } else {
      it(`safeParseInt - should return ${testCase.expected} for ${testCase.input}`, () => {
        expect(safeParseInt(testCase.input, testCase.bits, testCase.signed)).toEqual(
          testCase.expected
        );
      });
    }
  ...
  for (const testCase of safeParseFloatTestCases) {
    if (isNil(testCase.expected)) {
      it(`safeParseFloat - should throw for ${testCase.input}`, () => {
        expect(() => safeParseFloat(testCase.input, testCase.bits, testCase.signed)).toThrow();
      });
    } else {
      it(`safeParseFloat - should return ${testCase.expected} for ${testCase.input}`, () => {
        expect(safeParseFloat(testCase.input, testCase.bits, testCase.signed)).toEqual(
          testCase.expected
        );
      });
    }
