import { isString, safeParseInt } from './utils.js';

// TODO: should be able to use this to differntiate between int/uint

function calculateMaxValue(bits: number, signed: boolean) {
  return signed ? BigInt(2) ** BigInt(bits - 1) - BigInt(1) : BigInt(2) ** BigInt(bits) - BigInt(1);
}
...
class IntN {
  ...
  constructor(bits: number, value: bigint | boolean | number | string | IntN, signed: boolean) {
    this.bits = bits;
    this.signed = signed;
    const bigint = isString(value)
      ? safeParseInt(value, bits, signed)
      : BigInt(value instanceof IntN ? value.value : value);
    this.value = signed ? BigInt.asIntN(bits, bigint) : BigInt.asUintN(bits, bigint);
    if (
