...
import { FunctionDecl, newTypeVariable, OverloadDecl, VariableDecl } from './decls.js';
import { Overload } from './functions.js';
import {
  ADD_OPERATOR,
...
import { BoolRefVal, isBoolRefVal } from './types/bool.js';
import { ErrorRefVal } from './types/error.js';
import { IntRefVal } from './types/int.js';
import { stringContains, stringEndsWith, stringStartsWith } from './types/string.js';
import { Comparer } from './types/traits/comparer.js';
import { Container } from './types/traits/container.js';
import { Indexer } from './types/traits/indexer.js';
import { Matcher } from './types/traits/matcher.js';
import { Adder, Divider, Modder, Multiplier, Negater, Subtractor } from './types/traits/math.js';
...
export const stdFunctions = [
  ...
  new FunctionDecl({
    ...
    singleton: new Overload({
      ...
      binary: (lhs, rhs) => {
        ...
        if (cmp.equal(IntRefVal.IntOne).value() || cmp.equal(IntRefVal.IntZero).value()) {
  ...
  new FunctionDecl({
    ...
    singleton: new Overload({
      ...
      binary: (lhs, rhs) => {
        const cmp = (lhs as RefVal & Comparer).compare(rhs);
        if (cmp.equal(IntRefVal.IntNegOne).value() || cmp.equal(IntRefVal.IntZero).value()) {
          return BoolRefVal.True;
        }
