...
export function parseIntFromText(text: string) {
  ...
  try {
    ...
    throw new ParseException(e instanceof Error ? e.message : 'Integer literal is malformed', 0);
  }
  return value;
}
...
export function parseDoubleFromText(text: string) {
  ...
  try {
    ...
    throw new ParseException(e instanceof Error ? e.message : 'Double literal is malformed', 0);
  }
  return value;
}
...
function nextInts(iterator: IterableIterator<string>, count: number, scratch: number[]): boolean {
...
function checkForClosingQuote(text: string, quote: string): void {
  ...
  if (text.length < quote.length) {
    throw new ParseException(`String literal missing terminating quote ${quote}`, 0);
  }
  let position = 0;
  let isClosed = false;
        ...
        case '3':
          {
            ...
            let octalValue = nextCodePoint.codePointAt(0)! - '0'.codePointAt(0)!;
            octalValue = octalValue * 8 + (scratchCodePoints[0] - '0'.codePointAt(0)!);
            octalValue = octalValue * 8 + (scratchCodePoints[1] - '0'.codePointAt(0)!);
        ...
        case 'X':
          {
            ...
            if (!nextInts(iterator, 2, scratchCodePoints) || !areHexDigits(scratchCodePoints, 2)) {
        ...
        case 'u':
          {
            ...
            if (!nextInts(iterator, 4, scratchCodePoints) || !areHexDigits(scratchCodePoints, 4)) {
        ...
        case 'U':
          {
            ...
            if (!nextInts(iterator, 8, scratchCodePoints) || !areHexDigits(scratchCodePoints, 8)) {
              throw new ParseException('Invalid unicode escape sequence', seqOffset);
            }
