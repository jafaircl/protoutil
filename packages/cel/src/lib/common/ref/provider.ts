...
import { isType, Type } from '../types/types.js';
...
export function isFieldType(value: any): value is FieldType {
  return (
    value && isFunction(value['isSet']) && isFunction(value['getFrom']) && isType(value['type'])
  );
}
 ...
 */
type FieldGetter<TargetType = any, ValueType = any> = (target: TargetType) => ValueType;
