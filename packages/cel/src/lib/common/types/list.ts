...
import { Adapter } from '../ref/provider.js';
...
import { NativeType, reflectNativeType } from './native.js';
import { Foldable, Folder, isFoldable, Iterator } from './traits/iterator.js';
import { isLister, Lister } from './traits/lister.js';
import { Zeroer } from './traits/zeroer.js';
import { DoubleType, ErrorType, IntType, ListType, TypeType, UintType } from './types.js';
import { isValidUint64 } from './uint.js';
...
class BaseList<T = any> implements Lister, Zeroer {
  ...
  private _valuesToProtoValueValues() {
    ...
    for (const value of nativeValues) {
      switch (reflectNativeType(value)) {
        case Boolean:
          values.push(create(ValueSchema, { kind: { case: 'boolValue', value } }));
          break;
        case Number:
        case BigInt:
          values.push(
          ...
          break;
        case String:
          values.push(create(ValueSchema, { kind: { case: 'stringValue', value } }));
          break;
        // TODO: list, null, struct
        default:
          return new ErrorRefVal(`unsupported type '${reflectNativeType(value)}' in list`);
      }
    }
    return values;
  }
  ...
  contains(elem: RefVal): RefVal {
    for (let i = 0; i < this.size().value(); i++) {
      const val = this.adapter.nativeToValue(this.get(new IntRefVal(BigInt(i))));
  ...
  get(index: RefVal): RefVal {
    ...
    if ((i.value() as bigint) < 0 || (i.value() as bigint) >= this.size().value()) {
      return new ErrorRefVal(
        `index '${i.value()}' out of range in list size '${this.size().value()}'`
      );
    }
    return this.adapter.nativeToValue(this._value[Number((i as IntRefVal).value())]);
  }
  ...
  switch (index.type()) {
    ...
    case DoubleType:
      if (Math.round(index.value()) !== index.value()) {
        return new ErrorRefVal(`unsupported index value '${index.value()}' in list`);
      }
      retval = new IntRefVal(BigInt(index.value()));
      break;
    default:
      return new ErrorRefVal(`unsupported index type '${index.type().typeName()}' in list`);
  }
  if (isNil(retval)) {
    return new ErrorRefVal(`unsupported index type '${index.type().typeName()}' in list`);
  }
  if (!isValidUint64(retval.value())) {
    return new ErrorRefVal(`unsupported index value '${retval.value()}' in list`);
  }
  return retval;
}
