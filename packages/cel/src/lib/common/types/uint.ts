...
import { compareNumberRefVals } from './compare.js';
...
import { Adder, Divider, Modder, Multiplier, Subtractor } from './traits/math.js';
import { Zeroer } from './traits/zeroer.js';
import { DoubleType, ErrorType, IntType, StringType, TypeType, UintType } from './types.js';

export const MAX_UINT64 = BigInt(2) ** BigInt(64) - BigInt(1);

export function isValidUint32(value: number) {
  return !Number.isNaN(value) && value >= 0 && value <= MAX_UINT64 && value <= Infinity;
}

export function isValidUint64(value: bigint) {
  return !Number.isNaN(value) && value >= BigInt(0) && value <= MAX_UINT64 && value <= Infinity;
}

export class UintRefVal
  implements RefVal, Adder, Comparer, Divider, Modder, Multiplier, Subtractor, Zeroer
{
  ...
  convertToNative(type: NativeType) {
    switch (type) {
      ...
      case AnySchema:
        return anyPack(UInt64ValueSchema, create(UInt64ValueSchema, { value: this._value }));
  ...
  add(other: RefVal): RefVal {
    switch (other.type()) {
      ...
      case UintType:
        if (
          (other.value() > 0 && this._value > MAX_INT64 - BigInt(other.value())) ||
  ...
  compare(other: RefVal): RefVal {
    if (Number.isNaN(Number(this._value)) || Number.isNaN(Number(other?.value()))) {
