...
describe('object', () => {
  it('should create from the registry', () => {
    const registry = new Registry(new Map(), createMutableRegistry(ParsedExprSchema));
  ...
  it('isSet', () => {
    ...
    const obj = registry.nativeToValue(msg) as ObjectRefVal;
    expect(obj.isSet(new StringRefVal('source_info'))).toStrictEqual(BoolRefVal.True);
    ...
    expect(obj.isSet(IntRefVal.IntZero)).toStrictEqual(ErrorRefVal.errNoSuchOverload);
  });
  ...
  it('get', () => {
    ...
    const obj = registry.nativeToValue(msg) as ObjectRefVal;
    expect(
      obj.get(new StringRefVal('source_info')).equal(registry.nativeToValue(msg.sourceInfo))
    ).toStrictEqual(BoolRefVal.True);
    expect(obj.get(new StringRefVal('bad_field'))).toStrictEqual(
      new ErrorRefVal(`no such field 'bad_field'`)
    );
    expect(obj.get(IntRefVal.IntZero)).toStrictEqual(ErrorRefVal.errNoSuchOverload);
  });
