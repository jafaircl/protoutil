...
import { isMapper, Mapper, MutableMapper } from './traits/mapper.js';
import { DoubleType, IntType, MapType, TypeType, UintType } from './types.js';
...
export class RefValMap<K extends RefVal = RefVal, V extends RefVal = RefVal> extends BaseMap<K, V> {
  constructor(adapter: Adapter, value: Map<K, V>) {
    super(adapter, value);
  }

  override iterator(): Iterator {
    const keys = Array.from<RefVal>(this._value.keys());
  ...
  implements MutableMapper
{
  ...
  toImmutableMap(): Mapper {
    return new RefValMap(this.adapter, new Map(this._value as Map<RefVal, RefVal>));
  }
}
 ...
 */
export function toFoldableMap<K = any, V = any>(mapper: Mapper<K, V>): Foldable {
