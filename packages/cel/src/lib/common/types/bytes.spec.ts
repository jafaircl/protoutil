...
import { BoolType, BytesType, StringType, TypeType } from './types.js';

describe('bytes', () => {
  it('convertBytesValueToNative - js Uint8Array', () => {
    expect(new BytesRefVal(new Uint8Array([1, 2, 3])).convertToNative(Uint8Array)).toStrictEqual(
  ...
  it('convertBytesValueToType', () => {
    ...
    expect(value.convertToType(StringType)).toStrictEqual(new StringRefVal('helloworld'));
  ...
  it('equalBytesValue', () => {
    const value = new BytesRefVal(new Uint8Array([1, 2, 3]));
    expect(value.equal(new BytesRefVal(new Uint8Array([1, 2, 3])))).toStrictEqual(
      new BoolRefVal(true)
    );
    expect(value.equal(new BytesRefVal(new Uint8Array([1, 2, 4])))).toStrictEqual(
  ...
  it('isZeroBytesValue', () => {
    expect(new BytesRefVal(new Uint8Array([1, 2, 3])).isZeroValue()).toEqual(false);
    expect(new BytesRefVal(new Uint8Array([])).isZeroValue()).toEqual(true);
  });

  it('addBytesValue', () => {
    expect(
      new BytesRefVal(new Uint8Array([1, 2, 3])).add(new BytesRefVal(new Uint8Array([4, 5, 6])))
    ).toStrictEqual(new BytesRefVal(new Uint8Array([1, 2, 3, 4, 5, 6])));
    expect(
      new BytesRefVal(new Uint8Array([1, 2, 3])).add(new BytesRefVal(new Uint8Array([])))
    ).toStrictEqual(new BytesRefVal(new Uint8Array([1, 2, 3])));
    expect(
      new BytesRefVal(new Uint8Array([])).add(new BytesRefVal(new Uint8Array([1, 2, 3])))
    ).toStrictEqual(new BytesRefVal(new Uint8Array([1, 2, 3])));
    expect(new BytesRefVal(new Uint8Array([])).add(new StringRefVal('foo'))).toStrictEqual(
  ...
  it('compareBytesValue', () => {
    ...
    expect(
      new BytesRefVal(new Uint8Array([1, 2, 3, 4])).compare(new StringRefVal('foo'))
  ...
  it('sizeBytesValue', () => {
    ...
    expect(new BytesRefVal(new Uint8Array([])).size()).toStrictEqual(new IntRefVal(BigInt(0)));
  });
});
