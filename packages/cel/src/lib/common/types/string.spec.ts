...
import { AnySchema, StringValueSchema, anyPack, timestampFromDate } from '@bufbuild/protobuf/wkt';
import { duration } from '@protoutil/core';
import { CONTAINS_OVERLOAD } from '../overloads.js';
...
describe('string', () => {
  it('convertStringValueToNative ', () => {
    expect(new StringRefVal('hello').convertToNative(String)).toStrictEqual('hello');
    expect(new StringRefVal('hello').convertToNative(Uint8Array)).toStrictEqual(
      new TextEncoder().encode('hello')
    );
    expect(new StringRefVal('hey buddy').convertToNative(AnySchema)).toStrictEqual(
      anyPack(StringValueSchema, create(StringValueSchema, { value: 'hey buddy' }))
    );
    expect(new StringRefVal('hey buddy').convertToNative(StringValueSchema)).toStrictEqual(
  ...
  it('convertStringValueToType', () => {
    const tests = [
      ...
      },
      {
        ...
        type: TimestampType,
        expected: new TimestampRefVal(timestampFromDate(new Date('2021-01-01T00:00:00Z'))),
      },
      {
  ...
  it('equalStringValue', () => {
    expect(new StringRefVal('hello').equal(new StringRefVal('hello'))).toStrictEqual(
      new BoolRefVal(true)
    );
    expect(new StringRefVal('hello').equal(new StringRefVal('world'))).toStrictEqual(
  ...
  it('addStringValue', () => {
    expect(new StringRefVal('hello').add(new StringRefVal('world'))).toStrictEqual(
  ...
  it('compareStringValue', () => {
    ...
    expect(a.compare(new DoubleRefVal(42))).toStrictEqual(ErrorRefVal.errNoSuchOverload);
  });
  ...
  it('matchStringValue', () => {
    ...
    expect(new StringRefVal('ello 1 worlds').match(sw)).toStrictEqual(new BoolRefVal(false));
    expect(str.match(new DoubleRefVal(42))).toStrictEqual(ErrorRefVal.errNoSuchOverload);
  });
  ...
  it('receiveStringValue', () => {
    ...
    // Contains
    expect(
      new StringRefVal('goodbye').receive(CONTAINS_OVERLOAD, '', [new StringRefVal('db')])
    ).toStrictEqual(new BoolRefVal(true));
    expect(
      new StringRefVal('goodbye').receive(CONTAINS_OVERLOAD, '', [new StringRefVal('aa')])
    ).toStrictEqual(new BoolRefVal(false));
    expect(
      new StringRefVal('goodbye').receive(CONTAINS_OVERLOAD, '', [new DoubleRefVal(42)])
    ).toStrictEqual(ErrorRefVal.errNoSuchOverload);

    // StartsWith
    expect(
      new StringRefVal('goodbye').receive('startsWith', '', [new StringRefVal('good')])
    ).toStrictEqual(new BoolRefVal(true));
    expect(
      new StringRefVal('goodbye').receive('startsWith', '', [new StringRefVal('bye')])
    ).toStrictEqual(new BoolRefVal(false));
    expect(
      new StringRefVal('goodbye').receive('startsWith', '', [new DoubleRefVal(42)])
    ).toStrictEqual(ErrorRefVal.errNoSuchOverload);

    // EndsWith
    expect(
      new StringRefVal('goodbye').receive('endsWith', '', [new StringRefVal('bye')])
    ).toStrictEqual(new BoolRefVal(true));
    expect(
      new StringRefVal('goodbye').receive('endsWith', '', [new StringRefVal('good')])
    ).toStrictEqual(new BoolRefVal(false));
    expect(
      new StringRefVal('goodbye').receive('endsWith', '', [new DoubleRefVal(42)])
    ).toStrictEqual(ErrorRefVal.errNoSuchOverload);
  });

  it('sizeStringValue', () => {
    expect(new StringRefVal('').size()).toStrictEqual(new IntRefVal(BigInt(0)));
    expect(new StringRefVal('hello world').size()).toStrictEqual(new IntRefVal(BigInt(11)));
    expect(new StringRefVal('\u65e5\u672c\u8a9e').size()).toStrictEqual(new IntRefVal(BigInt(3)));
  });
});
