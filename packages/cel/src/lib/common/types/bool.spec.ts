...
describe('bool', () => {
  ...
  it('convertBoolValueToNative - anyPack', () => {
    const value = new BoolRefVal(true);
    const packed = anyPack(BoolValueSchema, create(BoolValueSchema, { value: true }));
  ...
  it('convertBoolValueToType', () => {
    ...
    expect(value.convertToType(StringType)).toStrictEqual(new StringRefVal('true'));
    expect(value.convertToType(TypeType)).toStrictEqual(BoolType);
  });

  it('equalBoolValue', () => {
    expect(BoolRefVal.True.equal(BoolRefVal.True)).toStrictEqual(BoolRefVal.True);
    expect(BoolRefVal.False.equal(BoolRefVal.True)).toStrictEqual(BoolRefVal.False);
    expect(BoolRefVal.True.equal(BoolRefVal.False)).toStrictEqual(BoolRefVal.False);
  });
  ...
  it('compareBoolValue', () => {
    expect(BoolRefVal.True.compare(BoolRefVal.True)).toStrictEqual(IntRefVal.IntZero);
    expect(BoolRefVal.False.compare(BoolRefVal.True)).toStrictEqual(IntRefVal.IntNegOne);
    expect(BoolRefVal.True.compare(BoolRefVal.False)).toStrictEqual(IntRefVal.IntOne);
