...
import { isNil, reverseMap } from '../utils.js';
...
import { Lister } from './traits/lister.js';
import { Mapper } from './traits/mapper.js';
import { AllTraits, Trait } from './traits/trait.js';
...
export class Type implements RefType, RefVal {
  ...
  declaredTypeName(): string {
    ...
    if (this.kind() != Kind.NULL && !this.isDyn() && this.isAssignableType(NullType)) {
  ...
  private _defaultIsAssignableRuntimeType(val: RefVal): boolean {
    ...
    switch (this.kind()) {
      ...
      case Kind.MAP:
        ...
        return keyType.isAssignableRuntimeType(keyVal) && valType.isAssignableRuntimeType(valVal);
      default:
        return true;
    }
  }
  ...
  protected _isTypeInternal(other: Type, checkTypeParamName: boolean): boolean {
    ...
    if (this.kind() !== other.kind() || this.parameters().length !== other.parameters().length) {
      return false;
    }
    if (
    ...
    for (let i = 0; i < this.parameters().length; i++) {
      if (!this.parameters()[i]._isTypeInternal(other.parameters()[i], checkTypeParamName)) {
        return false;
      }
    }
    return true;
...
export function isWellKnownType(type: Type) {
  switch (type.kind()) {
    ...
    case Kind.MAP:
      return type.parameters()[0] === StringType && type.parameters()[1] === DynType;
    default:
      return false;
  }
}
...
export function newNullableType(wrapped: Type) {
  return new Type({
    ...
    isAssignableRuntimeType: (other) =>
      NullType.isAssignableRuntimeType(other) || wrapped.isAssignableRuntimeType(other),
  });
}
  ...
  switch (t.kind()) {
    ...
    case Kind.LIST:
      if (t.parameters().length !== 1) {
        return new Error(`invalid list, got ${t.parameters().length} parameters, wanted one`);
      }
    ...
    case Kind.MAP:
      if (t.parameters().length !== 2) {
        return new Error(`invalid map, got ${t.parameters().length} parameters, wanted two`);
      }
 ...
 * representation.
 */
export function typeMapToProto(typeMap: Map<bigint, Type>): Record<string, ProtoType> {
