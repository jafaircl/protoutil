...
import { IntRefVal, isValidInt64 } from './int.js';
import { NativeType } from './native.js';
import { StringRefVal } from './string.js';
import { Comparer } from './traits/comparer.js';
import { Adder, Divider, Multiplier, Negater, Subtractor } from './traits/math.js';
...
export class DoubleRefVal
  implements RefVal, Adder, Comparer, Divider, Multiplier, Negater, Subtractor, Zeroer
{
  ...
  convertToNative(type: NativeType) {
    switch (type) {
      case Number:
        return this._value;
      case AnySchema:
        return anyPack(DoubleValueSchema, create(DoubleValueSchema, { value: this._value }));
