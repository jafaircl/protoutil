...
describe('uint', () => {
  it('convertUint64ValueToNative', () => {
    const tests = [
      ...
      },
      {
        ...
        type: AnySchema,
        output: anyPack(UInt64ValueSchema, create(UInt64ValueSchema, { value: BigInt(42) })),
      },
      {
      ...
      },
      {
        input: new UintRefVal(BigInt(5678)),
        type: String,
        output: ErrorRefVal.nativeTypeConversionError(new UintRefVal(BigInt(0)), String),
      },
    ];
  ...
  it('addUint64Value', () => {
    expect(new UintRefVal(BigInt(1)).add(new UintRefVal(BigInt(2)))).toStrictEqual(
      new UintRefVal(BigInt(3))
    );
    expect(new UintRefVal(BigInt(1)).add(new StringRefVal('2'))).toStrictEqual(
      ErrorRefVal.errNoSuchOverload
    );
    expect(new UintRefVal(BigInt(1)).add(new UintRefVal(MAX_INT64))).toStrictEqual(
      ErrorRefVal.errUintOverflow
    );
    expect(new UintRefVal(BigInt(1)).add(new IntRefVal(BigInt(-1000)))).toStrictEqual(
      ErrorRefVal.errUintOverflow
    );
    expect(new UintRefVal(MAX_INT64 - BigInt(1)).add(new UintRefVal(BigInt(1)))).toStrictEqual(
      new UintRefVal(MAX_INT64)
    );
  });
  ...
  it('divideUint64Value', () => {
    expect(new UintRefVal(BigInt(3)).divide(new UintRefVal(BigInt(2)))).toStrictEqual(
      new UintRefVal(BigInt(1))
    );
    expect(new UintRefVal(BigInt(3)).divide(new StringRefVal('2'))).toStrictEqual(
      ErrorRefVal.errNoSuchOverload
    );
    expect(new UintRefVal(BigInt(3)).divide(new UintRefVal(BigInt(0)))).toStrictEqual(
      ErrorRefVal.errDivideByZero
    );
  });

  it('moduloUint64Value', () => {
    expect(new UintRefVal(BigInt(21)).modulo(new UintRefVal(BigInt(2)))).toStrictEqual(
      new UintRefVal(BigInt(1))
    );
    expect(new UintRefVal(BigInt(3)).modulo(new StringRefVal('2'))).toStrictEqual(
      ErrorRefVal.errNoSuchOverload
    );
    expect(new UintRefVal(BigInt(3)).modulo(new UintRefVal(BigInt(0)))).toStrictEqual(
      ErrorRefVal.errModulusByZero
    );
  });

  it('multiplyUint64Value', () => {
    expect(new UintRefVal(BigInt(3)).multiply(new UintRefVal(BigInt(2)))).toStrictEqual(
      new UintRefVal(BigInt(6))
    );
    expect(new UintRefVal(BigInt(3)).multiply(new StringRefVal('2'))).toStrictEqual(
      ErrorRefVal.errNoSuchOverload
    );
    expect(new UintRefVal(BigInt(3)).multiply(new UintRefVal(BigInt(0)))).toStrictEqual(
      new UintRefVal(BigInt(0))
    );
    expect(new UintRefVal(MAX_INT64).multiply(new UintRefVal(BigInt(2)))).toStrictEqual(
      ErrorRefVal.errUintOverflow
    );
    expect(new UintRefVal(BigInt(42)).multiply(new IntRefVal(BigInt(-1)))).toStrictEqual(
      ErrorRefVal.errUintOverflow
    );
  });

  it('subtractUint64Value', () => {
    expect(new UintRefVal(BigInt(3)).subtract(new UintRefVal(BigInt(2)))).toStrictEqual(
      new UintRefVal(BigInt(1))
    );
    expect(new UintRefVal(BigInt(3)).subtract(new StringRefVal('2'))).toStrictEqual(
      ErrorRefVal.errNoSuchOverload
    );
    expect(new UintRefVal(BigInt(3)).subtract(new UintRefVal(BigInt(4)))).toStrictEqual(
      ErrorRefVal.errUintOverflow
    );
    expect(new UintRefVal(MAX_INT64).subtract(new IntRefVal(BigInt(-1)))).toStrictEqual(
      ErrorRefVal.errUintOverflow
    );
  });
});
