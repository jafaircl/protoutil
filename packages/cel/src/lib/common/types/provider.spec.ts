...
import { duration, timestamp } from '@protoutil/core';
import { DeclSchema, ReferenceSchema } from '../../protogen/cel/expr/checked_pb.js';
import {
...
describe('Registry', () => {
  ...
  it('register differing type definition conflict', () => {
    ...
    expect(err).toStrictEqual(
      new ErrorRefVal('type registration conflict. found: http.Request, input: http.Request')
    );
  });
  ...
  it('enumValue', () => {
    ...
    expect(enumValue.value().message).toEqual(`unknown enum name 'abc.def.GHI'`);

    // Valid cases
    enumValue = registry.enumValue('cel.expr.conformance.proto3.TestAllTypes.NestedEnum.FOO');
    expect(enumValue.value()).toEqual(BigInt(TestAllTypes_NestedEnum.FOO));
    enumValue = registry.enumValue('cel.expr.conformance.proto3.TestAllTypes.NestedEnum.BAR');
    expect(enumValue.value()).toEqual(BigInt(TestAllTypes_NestedEnum.BAR));
    enumValue = registry.enumValue('cel.expr.conformance.proto3.TestAllTypes.NestedEnum.BAZ');
    expect(enumValue.value()).toEqual(BigInt(TestAllTypes_NestedEnum.BAZ));

    // Valid enum but invalid value
    enumValue = registry.enumValue('cel.expr.conformance.proto3.TestAllTypes.NestedEnum.QUX');
  ...
  it('findStructType', () => {
    ...
    expect(celType?.parameters()[0].typeName()).toEqual('cel.expr.conformance.proto3.TestAllTypes');
  ...
  it('findStructFieldType', () => {
    ...
    for (const test of tests) {
      expect(registry.findStructFieldType(test.msgTypeName, test.fieldName)?.type).toStrictEqual(
        test.want
      );
    }
  });
  ...
  it('nativeToValue - primitives', () => {
    ...
    expect(registry.nativeToValue(['a', 'b', 'c']).value()).toStrictEqual(['a', 'b', 'c']);
    expect(registry.nativeToValue([new StringRefVal('a')])).toBeInstanceOf(RefValList);
    expect(registry.nativeToValue([new StringRefVal('a')]).value()).toStrictEqual([
      new StringRefVal('a'),
    ]);
  });
