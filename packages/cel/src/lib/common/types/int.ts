...
import { durationFromNanos, timestamp } from '@protoutil/core';
...
import { Adder, Divider, Modder, Multiplier, Negater, Subtractor } from './traits/math.js';
...
import { UintRefVal, isValidUint64 } from './uint.js';
...
export class IntRefVal
  implements RefVal, Adder, Comparer, Divider, Modder, Multiplier, Negater, Subtractor, Zeroer
{
  ...
  convertToNative(type: NativeType) {
    switch (type) {
      ...
      case AnySchema:
        return anyPack(Int64ValueSchema, create(Int64ValueSchema, { value: this._value }));
  ...
  subtract(subtrahend: RefVal): RefVal {
    switch (subtrahend.type()) {
      ...
      case UintType:
        if (
          (subtrahend.value() < 0 && this._value > MAX_INT64 + BigInt(subtrahend.value())) ||
          (subtrahend.value() > 0 && this._value < MIN_INT64 + BigInt(subtrahend.value()))
        ) {
          return ErrorRefVal.errIntOverflow;
        }
