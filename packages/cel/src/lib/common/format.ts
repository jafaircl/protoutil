...
import { newPrimitiveProtoType } from './pb/types.js';
...
export function formatCheckedType(t: ProtoType): string {
  switch (t.typeKind.case) {
    case 'dyn':
      return 'dyn';
    case 'function':
      return formatFunctionExprType(t.typeKind.value.resultType!, t.typeKind.value.argTypes, false);
    case 'listType':
    ...
    case 'wrapper':
      return `wrapper(${formatCheckedType(newPrimitiveProtoType(t.typeKind.value))})`;
    case 'error':
      return '!error!';
    case 'typeParam':
      return t.typeKind.value;
  ...
  switch (t.kind()) {
    ...
    case Kind.OPAQUE:
      if (t.typeName() === 'function') {
        ...
        // absent.
        return formatFunctionDeclType(t.parameters()[0], t.parameters().slice(1), false);
      }
      break;
...
export function formatFunctionExprType(
  ...
  return formatFunctionInternal(resultType, argTypes, isInstance, formatExprType);
}
