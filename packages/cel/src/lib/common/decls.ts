...
import { BinaryOp, FunctionOp, Overload, UnaryOp } from './functions.js';
import { newFunctionProto, newVarIdentDeclProto } from './pb/decls.js';
import { RefVal } from './ref/reference.js';
import { ErrorRefVal, isErrorRefVal } from './types/error.js';
import { Trait } from './types/traits/trait.js';
import { Kind, newTypeTypeWithParam, Type, typeToExprType } from './types/types.js';
import { isUnknownRefVal, mergeUnknowns, UnknownRefVal } from './types/unknown.js';
import { isUnknownOrError } from './types/utils.js';
 ...
 */
export function newFunction(name: string, ...opts: FunctionOpt[]): FunctionDecl | Error {
  ...
  merge(other: FunctionDecl) {
    ...
    for (const oID of other.overloadOrdinals) {
      ...
      try {
        ...
        throw new Error(`function declaration merge failed: ${(err as Error).message}`);
      }
    }
    if (!isNil(other.singleton)) {
      if (!isNil(merged.singleton) && !dequal(merged.singleton, other.singleton)) {
      ...
      for (const id of this.overloadOrdinals) {
        ...
        switch (args.length) {
          ...
          case 2:
            if (
              !isNil(o.binaryOp) &&
              o.matchesRuntimeBinarySignature(this.disableTypeGuards, args[0], args[1])
            ) {
 ...
 */
export function singletonUnaryBinding(fn: UnaryOp, traits: Trait[] = []): FunctionOpt {
  return (f: FunctionDecl) => {
    if (f.singleton) {
 ...
 * per-overload function bindings.
 */
export function singletonBinaryBinding(fn: BinaryOp, traits: Trait[] = []): FunctionOpt {
  return (f: FunctionDecl) => {
    if (f.singleton) {
      throw new Error(`function already has a singleton binding: ${f.name()}`);
    }
 ...
 * per-overload function bindings.
 */
export function singletonFunctionBinding(fn: FunctionOp, traits: Trait[] = []): FunctionOpt {
  return (f: FunctionDecl) => {
    if (f.singleton) {
      throw new Error(`function already has a singleton binding: ${f.name()}`);
    }
...
function newOverload(
  ...
  return (f) => {
    const overload = newOverloadInternal(overloadID, memberFunction, args, resultType, ...opts);
...
export class OverloadDecl {
  ...
  signatureOverlaps(other: OverloadDecl) {
    ...
    for (let i = 0; i < this.argTypes().length; i++) {
      ...
      argsOverlap =
        argsOverlap &&
        (thisArgType.isAssignableType(otherArgType) || otherArgType.isAssignableType(thisArgType));
    }
    return argsOverlap;
  }
  ...
  matchesRuntimeUnarySignature(disableTypeGuards: boolean, arg: RefVal) {
    return (
      matchRuntimeArgType(this.nonStrict, disableTypeGuards, this.argTypes()[0], arg) &&
   ...
   */
  matchesRuntimeBinarySignature(disableTypeGuards: boolean, arg1: RefVal, arg2: RefVal) {
    return (
      matchRuntimeArgType(this.nonStrict, disableTypeGuards, this.argTypes()[0], arg1) &&
      matchRuntimeArgType(this.nonStrict, disableTypeGuards, this.argTypes()[1], arg2) &&
  ...
  matchesRuntimeSignature(disableTypeGuards: boolean, ...args: RefVal[]) {
    ...
    for (let i = 0; i < args.length; i++) {
      if (!matchRuntimeArgType(this.nonStrict, disableTypeGuards, this.argTypes()[i], args[i])) {
        return false;
      }
    }
...
function matchOperandTraits(traits: Trait[], arg: RefVal) {
  for (const trait of traits) {
    if (trait === Trait.UNSPECIFIED || arg.type().hasTrait(trait) || isUnknownOrError(arg)) {
      continue;
    }
    return false;
  }
...
export function functionDeclToExprDecl(decl: FunctionDecl) {
  ...
  for (const overload of decl.overloadDecls()) {
    const argTypes = overload.argTypes().map((t) => typeToExprType(t)) as ProtoType[];
