...
describe('cel', () => {
  ...
  it('TestAbbrevsDisambiguation', () => {
    ...
    const [out, __, err] = interpret(env, `test ? dyn(Expr) : cel.expr.Expr{id: 1}`, {
      test: true,
      'external.Expr': 'string expr',
    });
    expect(err).toBeNull();
    expect(out?.value()).toEqual('string expr');
    const [out2, _, err2] = interpret(env, `test ? dyn(Expr) : cel.expr.Expr{id: 1}`, {
      test: false,
      'external.Expr': 'wrong expr',
    });
    expect(err2).toBeNull();
    expect(out2?.convertToNative(ExprSchema)).toStrictEqual(create(ExprSchema, { id: BigInt(1) }));
  });
  ...
  it('TestCustomEnv', () => {
    ...
    const iss = env.compile('a.b.c == true');
    expect(iss).toBeInstanceOf(Issues);
    expect(iss.toString()).toContain(`undeclared reference to '_==_' (in container '')`);
  ...
  describe('TestQuotedFields', () => {
    ...
    for (const tc of testCases) {
      it(`TestQuotedFields: ${tc.expr}`, () => {
        const [out, _, err] = interpret(new Env(enableIdentifierEscapeSyntax()), tc.expr, {});
        if (tc.out) {
          expect(err).toBeNull();
          expect(out?.value()).toEqual(tc.out.value());
        } else {
