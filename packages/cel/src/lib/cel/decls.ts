...
import { protoConstantToRefVal } from '../common/pb/constants.js';
import { isRefVal } from '../common/ref/reference.js';
...
import { Decl, DeclSchema, Type as ProtoType } from '../protogen/cel/expr/checked_pb.js';
...
export function exprDeclToDeclaration(decl: Decl) {
  switch (decl.declKind.case) {
    ...
    case 'ident':
      ...
      return constant(decl.name, t, protoConstantToRefVal(decl.declKind.value.value));
    default:
 ...
 */
export function protoDeclToDecl(decl: Decl): VariableDecl | FunctionDecl | Error {
  switch (decl.declKind.case) {
    case 'function':
    ...
    case 'ident':
      ...
      return newConstantDecl(decl.name, t, protoConstantToRefVal(decl.declKind.value.value));
    default:
      return new Error(`unsupported decl: ${decl.declKind.case}`);
  }
}
 ...
 */
export function maybeUnwrapDeclaration(decl: Declaration): VariableDecl | FunctionDecl {
