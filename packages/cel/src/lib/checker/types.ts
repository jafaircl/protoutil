...
export function isEqualOrLessSpecific(t1: Type, t2: Type): boolean {
  ...
  switch (kind1) {
    case Kind.OPAQUE:
      if (t1.typeName() !== t2.typeName() || t1.parameters().length !== t2.parameters().length) {
        return false;
      }
 ...
 */
export function isValidTypeSubstitution(m: Mapping, t1: Type, t2: Type): [boolean, boolean] {
  const kind1 = t1.kind();
  const kind2 = t2.kind();
 ...
 */
export function internalIsAssignableList(m: Mapping, l1: Type[], l2: Type[]): boolean {
 ...
 * assignable to l2.
 */
export function isAssignableList(m: Mapping, l1: Type[], l2: Type[]): Mapping | null {
  const mCopy = m.copy();
 ...
 */
export function notReferencedIn(m: Mapping, t: Type, withinType: Type): boolean {
...
export function substitute(m: Mapping, t: Type, typeParamToDyn: boolean): Type {
  ...
  switch (t.kind()) {
    case Kind.OPAQUE:
      return newOpaqueType(t.typeName(), ...substituteParams(m, t.parameters(), typeParamToDyn));
    case Kind.LIST:
...
export function substituteParams(m: Mapping, typeParams: Type[], typeParamToDyn: boolean): Type[] {
