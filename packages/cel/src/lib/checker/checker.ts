...
import { protoConstantToType } from '../common/pb/constants.js';
import {
  isMapProtoExpr,
  isStructProtoExpr,
...
import { AggregateLiteralElementType, Env } from './env.js';
...
export class Checker {
  ...
  checkConstExpr(e: Expr) {
    ...
    if (isNil(constant)) {
      this.#errors.reportInternalError(`expected constant expression, got ${e.exprKind.case}`);
      return;
    }
  ...
  checkIdentExpr(e: Expr) {
    ...
    if (isNil(exprIdent)) {
      this.#errors.reportInternalError(`expected ident expression, got ${e.exprKind.case}`);
      return;
    }
  ...
  checkSelectExpr(e: Expr) {
    ...
    if (isNil(sel)) {
      this.#errors.reportInternalError(`expected select expression, got ${e.exprKind.case}`);
      return;
    }
  ...
  checkOptSelect(e: Expr) {
    ...
    if (isNil(call)) {
      this.#errors.reportInternalError(`expected call expression, got ${e.exprKind.case}`);
      return;
    }
    ...
    if (isNil(fieldName)) {
      this.#errors.reportNotAnOptionalFieldSelection(e.id, this.location(e), field);
      return;
    }
  ...
  checkSelectField(e: Expr, operand: Expr, field: string, optional: boolean) {
    ...
    const operandType = substitute(this.#mapping, this.getType(operand)!, false);
    ...
    switch (targetType.kind()) {
      ...
      case Kind.STRUCT:
        ...
        const fieldType = this.lookupFieldType(e.id, messageType.typeName(), field);
        if (!isNil(fieldType)) {
      ...
      default:
        ...
        if (!isDynOrError(targetType)) {
          this.#errors.reportTypeDoesNotSupportFieldSelection(e.id, this.location(e), targetType);
        }
        resultType = DynType;
        break;
    }
  ...
  checkCall(e: Expr) {
    ...
    if (isNil(call.target)) {
      ...
      if (isNil(fn)) {
        this.#errors.reportUndeclaredReference(e.id, this.location(e), this.#env.container, fnName);
        this.setType(e, ErrorType);
        return;
      }
  ...
  resolveOverloadOrError(e: Expr, fn: FunctionDecl, target: Expr | null, args: Expr[]) {
  ...
  resolveOverload(
    ...
    if (!isNil(target)) {
      ...
      if (isNil(targetType)) {
        this.#errors.reportInternalError(`expected type for target expression, got ${target}`);
        return null;
      }
    ...
    for (const arg of args) {
      ...
      if (isNil(argType)) {
        this.#errors.reportInternalError(`expected type for argument expression, got ${arg}`);
        return null;
      }
    ...
    for (const overload of fn.overloadDecls()) {
      ...
      let overloadType = newFunctionType(overload.resultType(), ...overload.argTypes());
      ...
      if (this.isAssignableList(argTypes, candidateArgTypes)) {
        ...
        const fnResultType = substitute(this.#mapping, overloadType.parameters()[0], false);
        if (isNil(resultType)) {
  ...
  checkCreateList(e: Expr) {
    ...
    if (isNil(create)) {
      this.#errors.reportInternalError(`expected list expression, got ${e.exprKind.case}`);
      return;
    }
  ...
  checkCreateMap(e: Expr) {
    ...
    if (isNil(mapVal)) {
      this.#errors.reportInternalError(`expected map expression, got ${e.exprKind.case}`);
      return;
    }
  ...
  checkCreateStruct(e: Expr) {
    ...
    if (isNil(msgVal)) {
      this.#errors.reportInternalError(`expected proto message expression, got ${e.exprKind.case}`);
      return;
    }
    ...
    if (identKind !== Kind.ERROR) {
      if (identKind !== Kind.TYPE) {
        this.#errors.reportNotAType(e.id, this.location(e), ident.type().declaredTypeName());
      } else {
        ...
        if (isWellKnownType(resultType)) {
          ...
          this.#errors.reportNotAMessageType(e.id, this.location(e), resultType.declaredTypeName());
          resultType = ErrorType;
        }
      }
    }
  ...
  checkComprehension(e: Expr) {
    ...
    if (isNil(comp)) {
      this.#errors.reportInternalError(`expected comprehension expression, got ${e.exprKind.case}`);
      return;
    }
    this.checkExpr(comp.iterRange);
    this.checkExpr(comp.accuInit);
    const rangeType = substitute(this.#mapping, this.getType(comp.iterRange!)!, false);
    ...
    this.#env = this.#env.exitScope();
    this.setType(e, substitute(this.#mapping, this.getType(comp.result!)!, false));
  }
  ...
  joinTypes(expr: Expr, previous: Type, current: Type) {
    ...
    this.#errors.reportTypeMismatch(expr.id, this.location(expr), previous, current);
    return ErrorType;
  }

  dynAggregateLiteralElementTypesEnabled() {
    return this.#env.aggLitElemType === AggregateLiteralElementType.DYN_ELEMENT_TYPE;
  }
  ...
  setType(expr: Expr, t: Type) {
    ...
    if (!isNil(found) && !t.isExactType(found)) {
      this.#errors.reportIncompatibleTypes(expr.id, this.location(expr), expr, found, t);
    }
  ...
  assertType(e: Expr, t: Type) {
    ...
    if (isNil(et)) {
      this.#errors.reportInternalError(`expression type not set: ${e}(${e.id})`);
      return;
    }
  ...
  setReference(expr: Expr, ref: ReferenceInfo) {
    ...
    if (!isNil(found) && !found.equals(ref)) {
      this.#errors.reportReferenceRedefinition(expr.id, this.location(expr), expr, found, ref);
    }
  ...
  lookupFieldType(exprID: bigint, structType: string, fieldName: string): Type | null {
    const found = this.#env.provider.findStructType(structType);
    if (isNil(found)) {
      // This should not happen, anyway, report an error.
      this.#errors.reportUnexpectedFailedResolution(exprID, this.locationByID(exprID), structType);
      return null;
    }
    ...
    this.#errors.reportUndefinedField(exprID, this.locationByID(exprID), fieldName);
    return null;
  }
}
