...
import { newVariableDecl, VariableDecl } from '../common/decls.js';
import { CONTAINS_STRING_OVERLOAD, TIMESTAMP_TO_YEAR_OVERLOAD } from '../common/overloads.js';
...
import { macros, Parser } from '../parser/parser.js';
import {
...
describe('cost', () => {
  ...
  const testCases: TestCase[] = [
    ...
    },
    {
      ...
      vars: [newVariableDecl('input', newObjectType('cel.expr.conformance.proto3.TestAllTypes'))],
      wanted: new CostEstimate(BigInt(1), BigInt(1)),
    ...
    },
    {
      ...
      vars: [newVariableDecl('input', StringType), newVariableDecl('arg1', StringType)],
      hints: new Map([
        ['input', BigInt(500)],
        ['arg1', BigInt(500)],
      ]),
    ...
    },
    {
      ...
      vars: [newVariableDecl('str1', StringType), newVariableDecl('str2', StringType)],
      hints: new Map([
        ['str1', BigInt(10)],
        ['str2', BigInt(10)],
      ]),
    ...
    },
    {
      ...
      options: [
        overloadCostEstimate(CONTAINS_STRING_OVERLOAD, (estimator, target, args) => {
          if (!isNil(target) && args.length === 1) {
            const strSize = estimateSize(estimator, target).multiplyByCostFactor(0.2);
            const subSize = estimateSize(estimator, args[0]).multiplyByCostFactor(0.2);
