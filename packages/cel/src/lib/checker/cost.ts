...
import { MAX_UINT64 } from '../common/types/uint.js';
import { isNil } from '../common/utils.js';
import { AccumulatorName } from '../parser/macro.js';
...
export class AstNode {
  ...
  constructor(path: string[], type: Type, expr: Expr, derivedSize?: SizeEstimate) {
    this._path = path;
    this._type = type;
    this._expr = expr;
  ...
  #costIdent(e: Expr): CostEstimate {
    ...
    if (!isNil(iterRange)) {
      switch (this.#checkedAst.getType(iterRange)?.kind()) {
        case Kind.LIST:
          this.#addPath(e, [...(this.#exprPath?.get(iterRange) ?? []), '@items']);
          break;
        case Kind.MAP:
          this.#addPath(e, [...(this.#exprPath?.get(iterRange) ?? []), '@keys']);
          break;
        default:
          break;
      }
  ...
  #functionCost(
    ...
    if (this.overloadEstimators.size !== 0) {
      ...
      if (!isNil(estimator)) {
        ...
        return new CallEstimate(callEst.cost.add(argCostSum()), callEst.resultSize);
      }
    }
    const callEst = this.#estimator.estimateCallCost(fn, overloadID, target, args);
    if (!isNil(callEst)) {
      return new CallEstimate(callEst.cost.add(argCostSum()), callEst.resultSize);
    }
    switch (overloadID) {
        ...
        if (!isNil(target)) {
          ...
          return new CallEstimate(
            this.#sizeEstimate(target)
              ...
              .add(argCostSum() as unknown as SizeEstimate) as unknown as CostEstimate
          );
        }
        break;
      ...
      case CONTAINS_STRING_OVERLOAD:
        if (!isNil(target) && args.length === 1) {
          const strCost =
            this.#sizeEstimate(target).multiplyByCostFactor(StringTraversalCostFactor);
      ...
      case CONDITIONAL_OVERLOAD:
        const size = this.#sizeEstimate(args[1]).union(this.#sizeEstimate(args[2]));
    ...
    return new CallEstimate(new CostEstimate(BigInt(1), BigInt(1)).add(argCostSum()));
  }
