...
import { Provider } from '../common/ref/provider.js';
import { ErrorType, IntType, isType, newTypeTypeWithParam } from '../common/types/types.js';
...
export class Env {
  ...
  constructor(
    ...
    if (options?.homogeneousAggregateLiterals === true) {
      this.#aggLitElemType = AggregateLiteralElementType.HOMOGENEOUS_ELEMENT_TYPE;
    }
  ...
  exitScope() {
    return new Env(this.#container, this.#provider, {
      ...this.options,
      validatedDeclarations: this.#declarations.pop() ?? undefined,
    });
  }
}
