...
import { ExprHelper } from './helper.js';
 ...
 */
export type MacroExpander = (eh: ExprHelper, target: Expr | null, args: Expr[]) => Expr | CELError;
 ...
 */
export const ExistsOneMacro = new ReceiverMacro(EXISTS_ONE_MACRO, 2, makeExistsOne);
 ...
 */
function makeAll(eh: ExprHelper, target: Expr | null, args: Expr[]): Expr | CELError {
 ...
 */
function makeExists(eh: ExprHelper, target: Expr | null, args: Expr[]): Expr | CELError {
 ...
 */
function makeExistsOne(eh: ExprHelper, target: Expr | null, args: Expr[]): Expr | CELError {
 ...
 */
function makeMap(eh: ExprHelper, target: Expr | null, args: Expr[]): Expr | CELError {
  const v = extractIdent(args[0]);
  if (v === '') {
    return eh.newError(args[0].id, 'argument is not an identifier');
  }
  if (v === AccumulatorName) {
    return eh.newError(args[0].id, 'iteration variable overwrites accumulator variable');
  }
  ...
  return eh.newComprehension(target!, v, AccumulatorName, init, condition, step, eh.newAccuIdent());
}
 ...
 */
function makeFilter(eh: ExprHelper, target: Expr | null, args: Expr[]): Expr | CELError {
  const v = extractIdent(args[0]);
  if (v === '') {
    return eh.newError(args[0].id, 'argument is not an identifier');
  }
 ...
 * has(<operand>.field)
 */
function makeHas(eh: ExprHelper, target: Expr | null, args: Expr[]): Expr | CELError {
...
function makeQuantifier(kind: QuantifierKind, eh: ExprHelper, target: Expr | null, args: Expr[]) {
  const v = extractIdent(args[0]);
  if (v === '') {
  ...
  switch (kind) {
    ...
    case QuantifierKind.EXISTS_ONE:
      ...
      step = eh.newCall(
        CONDITIONAL_OPERATOR,
        args[1],
        eh.newCall(ADD_OPERATOR, eh.newAccuIdent(), eh.newLiteral(new IntRefVal(BigInt(1)))),
        eh.newAccuIdent()
      );
      result = eh.newCall(
        EQUALS_OPERATOR,
  ...
  return eh.newComprehension(target!, v, AccumulatorName, init, condition, step, result);
}
