...
class Unparser {
  ...
  private _visitConstVal(val: Constant) {
    switch (val.constantKind.case) {
      ...
      case 'stringValue':
        ...
        break;
      case 'durationValue':
        this._str += `duration("${toJsonString(DurationSchema, val.constantKind.value)}")`;
        break;
      case 'timestampValue':
        this._str += `timestamp("${toJsonString(TimestampSchema, val.constantKind.value)}")`;
        break;
      default:
  ...
  private _visitSelect(expr: Expr) {
    ...
    return this._visitSelectInternal(sel.operand as Expr, sel.testOnly, '.', sel.field);
  }

  private _visitSelectInternal(operand: Expr, testOnly: boolean, op: string, field: string) {
  ...
  return (unparser) => {
    ...
    for (const _symbol of symbols) {
      ...
      if (!found && _symbol !== CONDITIONAL_OPERATOR) {
        throw new Error(`Invalid unparser option. Unsupported operator: ${_symbol}`);
      }
      const _arity = arity(_symbol);
      if (_arity < 2) {
        throw new Error(`Invalid unparser option. Unary operators are unsupported: ${_symbol}`);
      }
