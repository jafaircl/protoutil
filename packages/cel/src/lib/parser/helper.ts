...
export class ParserHelper {
  ...
  newMapEntry(ctx: any, key: Expr, value: Expr | null, optionalEntry?: boolean) {
  ...
  newObjectField(ctx: any, name: string, value: Expr | null, optional?: boolean) {
  ...
  id(ctx: any) {
    ...
    if (ctx instanceof ParserRuleContext) {
      ...
      const prcStart = this._sourceInfo.computeOffset(prc.start!.line, prc.start!.column);
  ...
  buildMacroCallArg(expr: Expr): Expr {
    ...
    switch (expr.exprKind.case) {
      case 'callExpr':
        ...
        return newReceiverCallProtoExpr(expr.id, call.function, macroTarget, macroArgs);
      case 'listExpr':
        const list = expr.exprKind.value;
        const macroListArgs = list.elements.map((arg) => this.buildMacroCallArg(arg));
   ...
   */
  addMacroCall(exprID: bigint, fn: string, target: Expr | null, ...args: Expr[]) {
    const macroArgs = args.map((arg) => this.buildMacroCallArg(arg));
    if (isNil(target)) {
      this._sourceInfo.setMacroCall(exprID, newGlobalCallProtoExpr(BigInt(0), fn, macroArgs));
      return;
    }
...
export class LogicManager {
  ...
  static newVariadicLogicManager(functionName: string, term: Expr): LogicManager {
    return new LogicManager(functionName, term, true);
  }

  static newBalancingLogicManager(functionName: string, term: Expr): LogicManager {
...
export class ExprHelper {
  constructor(private readonly _id: bigint, private readonly _parserHelper: ParserHelper) {}
   ...
   */
  copy(expr: Expr): Expr {
    const offsetRange = this._parserHelper.getSourceInfo().getOffsetRange(expr.id);
    ...
    switch (expr.exprKind.case) {
      ...
      case 'callExpr':
        ...
        return newReceiverCallProtoExpr(copyID, call.function, this.copy(call.target), argsCopy);
      case 'listExpr':
        const list = expr.exprKind.value;
      ...
      case 'structExpr':
        ...
        if (isEmpty(expr.exprKind.value.messageName)) {
          ...
          const entriesCopy = m.entries.map((entry) => {
            ...
            return newMapEntryProtoExpr(copyID, key, value, entry.optionalEntry);
          });
  ...
  newMemberCall(functionName: string, target: Expr, ...args: Expr[]) {
    return newReceiverCallProtoExpr(this.nextMacroID(), functionName, target, args);
  }
