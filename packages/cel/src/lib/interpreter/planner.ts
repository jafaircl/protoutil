...
import { Attribute, AttributeFactory, Qualifier } from './attributes.js';
...
export class Planner implements InterpretablePlanner {
  ...
  planCheckedIdent(id: bigint, identRef: ReferenceInfo): Interpretable | Error {
    ...
    return new EvalAttr(this.#adapter, this.#attrFactory.absoluteAttribute(id, identRef.name!));
  }
  ...
  planSelect(expr: Expr): Interpretable | Error {
    ...
    const qual = this.#attrFactory.newQualifier(opType, expr.id, sel.field, false);
  ...
  planCallUnary(
    ...
    return new EvalUnary(expr.id, _function, overload, args[0], traits, fn, nonStrict);
  }
  ...
  planCallBinary(
    ...
    return new EvalBinary(expr.id, _function, overload, args[0], args[1], traits, fn, nonStrict);
  }
  ...
  planCallVarArgs(
    ...
    return new EvalVarArgs(expr.id, _function, overload, args, traits, fn, nonStrict);
  }
   ...
   * Interpretable.
   */
  planCallConditional(expr: Expr, args: Interpretable[]): Interpretable | Error {
    const cond = args[0];
    const t = args[1];
    let tAttr: Attribute;
   ...
   */
  planCallIndex(expr: Expr, args: Interpretable[], optional: boolean): Interpretable | Error {
    const op = args[0];
    const ind = args[1];
    ...
    if (isInterpretableConst(ind)) {
      qual = this.#attrFactory.newQualifier(opType, expr.id, ind.value(), optional);
  ...
  planCreateList(expr: Expr): Interpretable | Error {
    ...
    for (const index of optionalIndices) {
      if (index < 0 || index >= elements.length) {
        return new Error(`optional index ${index} out of element bounds [0, ${elements.length}]`);
      }
  ...
  planCreateMap(expr: Expr): Interpretable | Error {
    ...
    return new EvalMap(expr.id, keys, vals, optionals, hasOptionals, this.#adapter);
  }
  ...
  planCreateStruct(expr: Expr): Interpretable | Error {
    ...
    return new EvalObj(expr.id, typeName, fields, vals, optionals, hasOptionals, this.#provider);
  }
  ...
  planConst(expr: Expr): Interpretable | Error {
    ...
    return new EvalConst(expr.id, protoConstantToRefVal(unwrapConstantProtoExpr(expr)!));
  }
  ...
  resolveFunction(expr: Expr): [Expr | null, string, string] {
    ...
    if (isNil(target)) {
      ...
      for (const qualifiedName of this.#container.resolveCandidateNames(fnName)) {
    ...
    if (!isNil(qualifiedPrefix)) {
      ...
      for (const qualifiedName of this.#container.resolveCandidateNames(maybeQualifiedName)) {
        const overload = this.#disp.findOverload(qualifiedName);
        if (!isNil(overload)) {
   ...
   */
  relativeAttr(id: bigint, _eval: Interpretable, optional: boolean): Interpretable | Error {
    let eAttr = _eval as InterpretableAttribute;
    if (!isInterpretableAttribute(_eval)) {
      eAttr = new EvalAttr(this.#adapter, this.#attrFactory.relativeAttribute(id, _eval), optional);
    }
