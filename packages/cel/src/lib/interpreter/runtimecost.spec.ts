...
import { TIMESTAMP_TO_YEAR_OVERLOAD } from '../common/overloads.js';
...
import { HashMap, isNil, objectToMap } from '../common/utils.js';
  ...
  describe('TestRuntimeCost', () => {
    ...
    const testCases: TestCase[] = [
      ...
      },
      {
        ...
        vars: [newVariableDecl('input', newObjectType('google.api.expr.proto3.test.TestAllTypes'))],
        want: BigInt(1),
    ...
    for (const tc of testCases) {
      it(tc.name, () => {
        ...
        const [cost, est] = computeCost(tc.expr, tc.vars ?? [], ctx, tc.options ?? []);
...
function computeCost(
  ...
  const costTracker = new CostTracker(new TestRuntimeCostEstimator(), ...options);
