...
import { decObserveEval, InterpretableDecorator } from './decorators.js';
...
export interface Interpreter {
   ...
   */
  newInterpretable(exprAST: AST, ...decorators: InterpretableDecorator[]): Interpretable | Error;
}
 ...
 */
export type EvalObserver = (id: bigint, programStep: any, value: RefVal) => void;
...
export class ExprInterpreter implements Interpreter {
  ...
  newInterpretable(checked: AST, ...decorators: InterpretableDecorator[]): Interpretable | Error {
    const p = new Planner(
      this.#dispatcher,
      this.#provider,
      this.#adapter,
