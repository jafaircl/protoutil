...
import { OptionalNone, OptionalRefVal } from '../common/types/optional.js';
import { Registry } from '../common/types/provider.js';
import { IntType, newObjectType, Type } from '../common/types/types.js';
import { UnknownRefVal } from '../common/types/unknown.js';
...
import { EmptyActivation, MapActivation } from './activation.js';
import { AttrFactory, AttributeFactory, Qualifier } from './attributes.js';
import { EvalConst } from './interpretable.js';
...
describe('attributes', () => {
  ...
  it('TestAttributesRelativeAttrRelativeQualifier', () => {
    ...
    const qualB = makeQualifier(attrs, null, BigInt(5), attrs.absoluteAttribute(BigInt(5), 'b'));
  ...
  describe('TestAttributesOptional', () => {
    ...
    const tests: Test[] = [
      ...
      {
        name: 'a.b[1] with no value where b is a map[uint]',
        varName: 'a',
        quals: ['b', BigInt(1)],
        vars: objectToMap({
          a: {
      ...
      {
        name: `a.b[?1] with no value where 'b' is a []int`,
        varName: 'a',
        quals: ['b'],
        optQuals: [BigInt(1)],
        vars: objectToMap({
      ...
      {
        name: `a.b[1] with no value where 'b' is a map[int]any`,
        varName: 'a',
        quals: ['b', BigInt(1)],
        vars: objectToMap({
          a: {
      ...
      // TODO: this test case is broken. It's the only thing that is though so commenting it out for now.
      // {
      //   name: `a.b[0][false] where 'a' is optional`,
      //   varName: 'a',
      ...
      // },
      {
        name: `a.b[0][false] where 'a' is optional none.`,
        varName: 'a',
        quals: ['b', BigInt(0), false],
        vars: objectToMap({
          a: OptionalNone,
      ...
      {
        name: `a[?b] where 'b' is dynamically computed.`,
        varName: 'a',
      ...
      {
        name: 'a[c.d.e] where the c.d.e errors',
        varName: 'a',
        quals: [
          attrs
      ...
      {
        name: 'a.?single_int32 with a value.',
        varName: 'a',
      ...
      {
        name: 'a.?single_int32 where the field is set (uses more optimal selection logic)',
        varName: 'a',
        optQuals: [
          makeOptQualifier(
            attrs,
    ...
    for (let i = 0; i < tests.length; i += 1) {
      const tc = tests[i];
      it(`TestAttributesOptional ${i + 1}: ${tc.name}`, () => {
        let j = BigInt(0);
  ...
  it('BenchmarkResolverFieldQualifier', () => {
    ...
    const opType = reg.findStructType('cel.expr.conformance.proto3.TestAllTypes');
    const fieldType = reg.findStructType('cel.expr.conformance.proto3.TestAllTypes.NestedMessage');
    attr.addQualifier(makeQualifier(attrs, opType, BigInt(2), 'single_nested_message'));
  ...
  it('TestAttributesMissingMsg', () => {
    ...
    const out = attr.resolve(vars);
    expect(out).toEqual(new Error(`unknown type: 'cel.expr.conformance.proto3.TestAllTypes'`));
  });
