...
export interface ActualCostEstimator {
  callCost(fn: string, overloadID: string, args: RefVal[], result: RefVal): bigint | null;
}
...
export class CostTracker {
  ...
  constructor(estimator: ActualCostEstimator | null, ...options: CostTrackerOption[]) {
  ...
  costCall(call: InterpretableCall, args: RefVal[], result: RefVal): bigint {
    ...
    if (!isNil(this.estimator)) {
      const callCost = this.estimator.callCost(call.function(), call.overloadID(), args, result);
      if (!isNil(callCost)) {
        cost += callCost;
        return cost;
      }
    ...
    switch (call.overloadID()) {
      ...
      case EXT_FORMAT_STRING_OVERLOAD:
        cost += multiplyByCostFactor(this.actualSize(args[0]), StringTraversalCostFactor);
        break;
      ...
      case CONTAINS_STRING_OVERLOAD:
        const _strCost = multiplyByCostFactor(this.actualSize(args[0]), StringTraversalCostFactor);
