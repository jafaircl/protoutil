...
import { Adapter, FieldType, Provider } from '../common/ref/provider.js';
...
import { indexOrError, RefValList } from '../common/types/list.js';
import { RefValMap } from '../common/types/map.js';
import { reflectNativeType } from '../common/types/native.js';
import { isOptionalRefVal, OptionalNone, OptionalRefVal } from '../common/types/optional.js';
...
export function isNamespacedAttribute(value: any): value is NamespacedAttribute {
  return (
    value &&
 ...
 */
export function enableErrorOnBadPresenceTest(value: boolean): AttrFactoryOption {
  return (fac) => {
...
export class AttrFactory implements AttributeFactory {
  ...
  newQualifier(objType: Type, qualID: bigint, val: any, opt: boolean): Qualifier | Error {
    ...
    return newQualifier(this.#adapter, qualID, val, opt, this.errorOnBadPresenceTest);
  }
}
...
class ConditionalAttribute implements Attribute {
  ...
  toString() {
    return `id: ${this.#id}, truthy attribute: ${this.truthy}, falsy attribute: ${this.falsy}`;
  }
}
...
class MaybeAttribute implements Attribute {
  ...
  addQualifier(qual: Qualifier): Attribute | Error {
    ...
    if (augmentedNames.length > 0) {
      this.#attrs = [this.#fac.absoluteAttribute(this.#id, ...augmentedNames), ...this.#attrs];
    }
    return this;
  }
...
function newQualifier(
  ...
  switch (reflectNativeType(v)) {
    ...
    default:
      ...
      return new Error(`invalid qualifier type: ${adapter.nativeToValue(v).type().typeName()}`);
  }
}
...
class StringQualifier implements ConstantQualifier {
  ...
  private _qualifyInternal(
    ...
    return [null, false, new ResolutionError(undefined, undefined, this.#celValue)];
  }
}
...
class FieldQualifier implements ConstantQualifier {
  ...
  constructor(id: bigint, name: string, fieldType: FieldType, adapter: Adapter, optional: boolean) {
    this.#id = id;
    this.#name = name;
...
function attrQualify(
  ...
  const qual = fac.newQualifier(null, qualAttr.id(), val, qualAttr.isOptional());
