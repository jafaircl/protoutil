...
import { BinaryOp, FunctionOp, UnaryOp } from '../common/functions.js';
import { EQUALS_OPERATOR, NOT_EQUALS_OPERATOR } from '../common/operators.js';
import { EQUALS_OVERLOAD, NOT_EQUALS_OVERLOAD } from '../common/overloads.js';
import { Adapter, Provider } from '../common/ref/provider.js';
import { isRefVal, RefVal } from '../common/ref/reference.js';
import { BoolRefVal, isBoolRefVal } from '../common/types/bool.js';
import { ErrorRefVal, isErrorRefVal, labelErrorNode, wrapError } from '../common/types/error.js';
import { MutableList, toFoldableList } from '../common/types/list.js';
import { isOptionalRefVal } from '../common/types/optional.js';
import { Foldable, Iterable } from '../common/types/traits/iterator.js';
import { isLister, isMutableLister } from '../common/types/traits/lister.js';
import { isMapper, isMutableMapper } from '../common/types/traits/mapper.js';
import { Receiver } from '../common/types/traits/receiver.js';
import { Trait } from '../common/types/traits/trait.js';
import { BoolType, ListType, MapType, newObjectType, Type } from '../common/types/types.js';
import { isUnknownRefVal, maybeMergeUnknowns, UnknownRefVal } from '../common/types/unknown.js';
...
export function isInterpretableAttribute(value: any): value is InterpretableAttribute {
  return (
    value &&
...
export function isInterpretableConstructor(value: any): value is InterpretableConstructor {
...
export class EvalTestOnly implements InterpretableAttribute {
  ...
  addQualifier(qual: Qualifier): Attribute | Error {
    if (!isConstantQualifier(qual)) {
      return new Error(`test only expressions must have constant qualifiers: ${qual}`);
    }
  ...
  eval(ctx: Activation) {
    ...
    if (argVal.type().hasTrait(Trait.RECEIVER_TYPE)) {
      return labelErrorNode(
        this.#id,
        (argVal as RefVal & Receiver).receive(this.#function, this.#overload, [])
      );
    }
...
export class EvalBinary implements InterpretableCall {
  ...
  eval(ctx: Activation) {
    ...
    if (lVal.type().hasTrait(Trait.RECEIVER_TYPE)) {
      return labelErrorNode(
        this.#id,
        (lVal as RefVal & Receiver).receive(this.#function, this.#overload, [rVal])
      );
    }
    return new ErrorRefVal(`no such overload: ${this.#function}`, this.#id);
  }
...
export class EvalVarArgs implements InterpretableCall {
  ...
  eval(ctx: Activation) {
    ...
    if (arg0.type().hasTrait(Trait.RECEIVER_TYPE)) {
      return labelErrorNode(
        this.#id,
        (arg0 as RefVal & Receiver).receive(this.#function, this.#overload, argVals.slice(1))
      );
    }
    return new ErrorRefVal(`no such overload: ${this.#function}`, this.#id);
  }
    ...
    for (let i = 0; i < this.#keys.length; i++) {
      ...
      if (this.#hasOptionals && this.#optionals[i]) {
        if (!isOptionalRefVal(valVal)) {
          return labelErrorNode(this.#id, invalidOptionalEntryInit(keyVal, valVal));
        }
        if (!valVal.hasValue()) {
...
export class EvalObj implements InterpretableConstructor {
  ...
  eval(ctx: Activation) {
    ...
    return labelErrorNode(this.#id, this.#provider.newValue(this.#typeName, fieldVals));
  }
  ...
  eval(ctx: Activation) {
    ...
    if (this.iterVar2 != '') {
      ...
      if (isMapper(foldRange)) {
        ...
        return new ErrorRefVal(`unsupported comprehension range type: ${foldRange}`, this.#id);
      }
      foldable?.fold(f);
      return f.evalResult();
    }
    ...
    if (!foldRange.type().hasTrait(Trait.ITERABLE_TYPE)) {
      return ErrorRefVal.valOrErr(foldRange, `got '${foldRange}', expected iterable type`);
    }
...
export class EvalWatchAttr implements InterpretableAttribute {
  ...
  constructor(interpretableAttr: InterpretableAttribute, observer: EvalObserver) {
  ...
  addQualifier(qual: Qualifier): Attribute | Error {
    ...
    if (isConstantQualifier(qual)) {
      ...
      q = new EvalWatchAttrQual(qual.interpretableAttr, this.#observer, this.adapter());
...
export class EvalWatchConstQual implements ConstantQualifier {
  ...
  constructor(constantQualifier: ConstantQualifier, observer: EvalObserver, adapter: Adapter) {
  ...
  qualifyIfPresent(
    ...
    const [out, present, err] = this.#constantQualifier.qualifyIfPresent(vars, obj, presenceOnly);
    let val: RefVal;
    if (!isNil(err)) {
...
export class EvalWatchAttrQual implements Attribute {
  ...
  qualifyIfPresent(
    ...
    const [out, present, err] = this.#attr.qualifyIfPresent(vars, obj, presenceOnly);
    let val: RefVal;
    if (!isNil(err)) {
      val = labelErrorNode(this.id(), wrapError(err));
    } else if (!isNil(out)) {
...
export class EvalWatchQual implements Qualifier {
  ...
  qualifyIfPresent(
    ...
    const [out, present, err] = this.#qual.qualifyIfPresent(vars, obj, presenceOnly);
    let val: RefVal;
    if (!isNil(err)) {
      val = labelErrorNode(this.id(), wrapError(err));
    } else if (!isNil(out)) {
  ...
  foldIterable(iterable: Iterable): RefVal {
    ...
    while (it.hasNext().value()) {
      ...
      if (isBoolRefVal(cond)) {
        if (this.#interrupted || (!this.#evalFold.exhaustive && cond.value() !== true)) {
  ...
  resolveName(name: string): any {
    ...
    if (!this.#computeResult) {
      if (name == this.#evalFold.iterVar) {
        this.#iterVar1Val = this.#evalFold.adapter.nativeToValue(this.#iterVar1Val);
        return this.#iterVar1Val;
      }
      if (name == this.#evalFold.iterVar2) {
        this.#iterVar2Val = this.#evalFold.adapter.nativeToValue(this.#iterVar2Val);
