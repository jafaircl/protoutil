...
describe("interpreter/attribute_patterns_test.go", () => {
    ...
    it("matches equivalent field, string, boolean, and numeric qualifier values", () => {
      ...
      expect(
        attributePattern("a")
          .qualString("hello")
          .qualifierPatterns()[0]
          ?.matches(fac.qualifier({ id: 1, value: "hello", optional: false })),
      ).toBe(true);
      expect(
        attributePattern("a")
          .qualBool(true)
          .qualifierPatterns()[0]
          ?.matches(fac.qualifier({ id: 1, value: true, optional: false })),
      ).toBe(true);
      expect(
        attributePattern("a")
          .qualInt(42)
          .qualifierPatterns()[0]
          ?.matches(fac.qualifier({ id: 1, value: 42n, optional: false })),
      ).toBe(true);
    });
  });

  describe("interpreter/attribute_patterns_test.go/TestPartialAttributeFactory_MaybeAttributeGloballyNamespaced", () => {
  ...
  describe("interpreter/attribute_patterns_test.go/TestPartialAttributeFactory_ResolveUnknownQualifier", () => {
