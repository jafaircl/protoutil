...
import { parsedFloat, parsedInt } from './parsedexpr.js';
...
describe('Parser', () => {
  const testCases = [
    ...
    },
    {
      ...
      expected: and(func('msg.endsWith', string('world')), lessThan(text('retries'), int(10))),
    },
    // TODO: member calls
    // {
    //   filter: `math.mem('30mb')`,
    //   expected: instanceFunc('mem', text('math'), string('30mb')),
    // },
    // {
    //   filter: `(msg.endsWith('world') AND retries < 10)`,
    //   expected: and(
    //     instanceFunc('endsWith', text('msg'), string('world')),
  ...
  for (const testCase of testCases) {
    it(`should parse "${testCase.filter}"`, () => {
      ...
      if (result instanceof Error) {
        expect(drillDownOnErrorMessage(result)).toContain(testCase.errorContains);
        return;
      }
  ...
  it('parseNumger', () => {
    expect(new Parser('123.456').parseNumber()).toEqual(parsedFloat(BigInt(0), 123.456));
    expect(new Parser('-123').parseNumber()).toEqual(parsedInt(BigInt(0), -BigInt(123)));
  });

  it('parseFloat', () => {
    expect(new Parser('123.456').parseFloat()).toEqual(parsedFloat(BigInt(0), 123.456));
    expect(new Parser('-123.456').parseFloat()).toEqual(parsedFloat(BigInt(0), -123.456));
    expect(new Parser('.456').parseFloat()).toEqual(parsedFloat(BigInt(0), 0.456));
    expect(new Parser('-.456').parseFloat()).toEqual(parsedFloat(BigInt(0), -0.456));
    expect(new Parser('123.').parseFloat()).toEqual(parsedFloat(BigInt(0), 123.0));
  });

  it('parseInt', () => {
    expect(new Parser('123').parseInt()).toEqual(parsedInt(BigInt(0), BigInt(123)));
    expect(new Parser('-123').parseInt()).toEqual(parsedInt(BigInt(0), -BigInt(123)));
    expect(new Parser('0x123').parseInt()).toEqual(parsedInt(BigInt(0), BigInt(291)));
    expect(new Parser('-0x123').parseInt()).toEqual(parsedInt(BigInt(0), -BigInt(291)));
  });
});
