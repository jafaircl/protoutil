...
import { calculateRequestCheckSum, RequestMessage } from './request.js';
...
export function parsePageToken<T extends string = string>(
  ...
  if (!request.pageToken || isScalarZeroValue(ScalarType.STRING, request.pageToken)) {
    let offset = 0;
    if (request.skip) {
  ...
  if (pageToken.requestChecksum !== requestCheckSum) {
    throw new InvalidArgumentError({
      ...
      errorInfo: {
        ...
        domain: 'protoutil.aip.pagination',
        metadata: {
