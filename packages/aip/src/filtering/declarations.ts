...
import { type Constant, ConstantSchema } from "../gen/google/api/expr/v1alpha1/syntax_pb.js";
 ...
 */
export function newFunctionDeclaration(name: string, ...overloads: Decl_FunctionDecl_Overload[]) {
 ...
 */
export function newFunctionOverload(id: string, result: Type, ...params: Type[]) {
 ...
 */
export function newConstantDeclaration(name: string, type: Type, value: Constant) {
  return create(DeclSchema, {
    name,
    declKind: {
      case: "ident",
...
export class Declarations {
    ...
    for (const value of values) {
      if (!this._idents.has(value.name)) {
        this.declareConstant(
          newConstantDeclaration(value.name, enumIdentType, newStringConstant(value.name)),
        );
      }
    }
  }
  ...
  declareFunction(decl: Decl) {
    ...
    const existingOverloads = (fn.declKind.value as Decl_FunctionDecl).overloads ?? [];
    ...
    for (const newOverload of newOverloads) {
      ...
      if (!equals(Decl_FunctionDecl_OverloadSchema, newOverload, existingOverload)) {
